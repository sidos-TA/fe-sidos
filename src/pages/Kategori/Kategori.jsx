...
const Kategori = () => {
  ...
  const [loadingSetStateAHP, setLoadingSetStateAHP] = useState(false);

  const updateKategoriHandler = () => {
    const arrDatas = [];
    for (let i = 0; i < form?.getFieldValue("skalaKriteria")?.length; i++) {
      arrDatas?.push({
        id: `K${i + 1}`,
        name: form?.getFieldValue(`kriteria${i + 1}`),
        benefitCost: form?.getFieldValue(`benefitCost${i + 1}`),
        code: `K${i + 1}`,
        skala: JSON.stringify(
          form?.getFieldValue("skalaKriteria")[i]?.skala || "{}"
        ),
        stringSkala: JSON.stringify(
          form?.getFieldValue("skalaKriteria")[i]?.stringSkala || "{}"
        ),
